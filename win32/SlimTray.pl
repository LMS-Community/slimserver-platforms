...
sub ToolTip {
	...
	if ($ssActive and $scanning) {
		return "SlimServer Running (Scanning)";
	}
	elsif ($ssActive) {
...
sub checkSSActive {
	...
	# Use the Win32::Service package to see if the service is still active.
	...
	if ($status{'CurrentState'} == 0x04) {
		...
		# Look for the SlimServer CLI, if we're running as an application.
