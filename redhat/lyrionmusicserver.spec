   ...
   if [ -n "$currentPkg" ]; then
      ...
      if [ "$name" = "logitechmediaserver" ]; then
         ...
         if [[ -f /var/lib/squeezeboxserver/prefs/server.prefs ]]; then
            ...
            echo "your old configuration yourself."
            echo "#######################################################################"
         fi
...
if [ -f /var/tmp/migrateSqueezeboxserverConfig ]; then
   if ! /usr/bin/cp -pr /var/lib/squeezeboxserver/* /var/lib/%{shortname} 2>&1; then
      echo "WARNING, failed migrating old configuration. You will need to migrate it yourself."
   else
      /usr/bin/find /var/lib/%{shortname} -type f -name "*.prefs" -exec sed -i 's#/squeezeboxserver#/%{shortname}#g' {} \;
      /usr/bin/chown -R %{userd}:%{groupd} /var/lib/%{shortname}
   fi
...
-  As from version 9.0.0 the logitechmediaserver is called Lyrion Music Server.
   Re-branding everything to Lyrion Music server.
     ...
     - When a logitechmediaserver installation is upgraded to a lyrionmusicserver
       installation, then the configuration of the logitechmediaserver
       installation will be migrated to the lyrionmusicserver if the config is
       in the default location and /var/lib/lyrionmusicserver does not already
       exist. If these conditions are not met, then the lyrionmusicserver will
