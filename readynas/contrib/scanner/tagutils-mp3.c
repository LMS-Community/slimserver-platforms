...
_get_mp3tags(char *file, struct song_metadata *psong)
{
  ...
  if (!pid3tag) {
    ...
    errno = err;
    DPRINTF(E_WARN, L_SCAN_SCANNER, "Cannot get ID3 tag for %s\n", file);
    return -1;
  }
  ...
  id3_file_close(pid3file);
  DPRINTF(E_INFO, L_SCAN_SCANNER, "Got id3 tag successfully for file=%s\n", file);
  return 0;
}
...
static void _mp3_get_average_bitrate(FILE *infile, struct mp3_frameinfo *pfi)
{
  ...
  while (!found) {
    ...
    if (index >= (sizeof(frame_buffer)-4)) { // max mp3 framesize = 2880
      DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Could not find frame... quitting\n");
      return;
    }
	...
	DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Could not read frame header\n");
	return;
      }
  ...
  while (frame_count < 10) {
    ...
    if (_decode_mp3_frame(header,&fi)) {
      DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Invalid frame header while averaging\n");
      return;
    }
...
_mp3_get_frame_count(FILE *infile, struct mp3_frameinfo *pfi)
{
  ...
  while (1) {
	...
	if (cbr && (frames > 100)) {
	  DPRINTF(E_DEBUG, L_SCAN_SCANNER, "File appears to be CBR... quitting frame _mp3_get_frame_count()\n");
	  return;
	}
      }
    }
	...
	DPRINTF(E_ERROR, L_SCAN_SCANNER, "Frame count aborted on error.  Pos=%d, Count=%d\n",
		pos, frames);
	return;
      }
    }
...
_get_mp3fileinfo(char *file, struct song_metadata *psong)
{
  ...
  if (fread(buffer, 1, sizeof(buffer), infile) != sizeof(buffer)) {
    if (ferror(infile)) {
      DPRINTF(E_ERROR, L_SCAN_SCANNER, "Error reading: %s\n", strerror(errno));
    }
    else {
      DPRINTF(E_ERROR, L_SCAN_SCANNER, "File too small. Probably corrupted.\n");
    }
    fclose(infile);
    return -1;
  }
  ...
  while (!found) {
	...
	if (!strncasecmp((char*)&buffer[index+fi.xing_offset+4], "XING",4)) {
	  ...
	  if (fread(frame_buffer, 1, sizeof(frame_buffer), infile) == sizeof(frame_buffer)) {
	    ...
	    DPRINTF(E_ERROR, L_SCAN_SCANNER, "Could not read frame header: %s\n",file);
	    fclose(infile);
	    return 0;
	  }
	...
	index++;
	if (first_check) {
	  DPRINTF(E_WARN, L_SCAN_SCANNER, "Bad header... dropping back for full frame search\n");
	  first_check = 0;
	  fp_size = 0;
	  break;
	}
  ...
  if (_decode_mp3_frame(&buffer[index], &fi)) {
    fclose(infile);
    DPRINTF(E_ERROR, L_SCAN_SCANNER, "Could not find sync frame: %s\n", file);
    return 0;
  }
  ...
  fclose(infile);
  DPRINTF(E_INFO, L_SCAN_SCANNER, "Got fileinfo successfully for file=%s song_length=%d\n",
