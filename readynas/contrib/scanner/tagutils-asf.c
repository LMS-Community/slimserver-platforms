...
_asf_read_stream_object(FILE *fp, struct song_metadata *psong, __u32 size)
{
  ...
  if (IsEqualGUID(&s.StreamType, &ASF_AudioStream))
    ...
    DPRINTF(E_ERROR, L_SCAN_SCANNER, "Unknown asf stream type.\n");
  }
...
_asf_load_picture(FILE *fp, int size, void *bm, int *bm_size)
{
  ...
  if (!strcasecmp(buf, "image/jpeg") ||
    ...
    if (size > 0) {
      if (!(bm = malloc(size))) {
	DPRINTF(E_ERROR, L_SCAN_SCANNER, "Couldn't allocate %d bytes\n", size);
      }
      else {
	*bm_size = size;
	if (size <= *bm_size) {
	  ...
	  DPRINTF(E_ERROR, L_SCAN_SCANNER, "Overrun %d bytes required\n", size);
	  free(bm);
	  bm = NULL;
	}
      }
      ...
      DPRINTF(E_ERROR, L_SCAN_SCANNER, "No binary data\n");
      size = 0;
      bm = NULL;
    }
  }
