		    ...
		    struct song_metadata *psong)
{
  ...
  while (ogg_stream_packetout(&stream->os, &packet) > 0) {
	...
	DPRINTF(E_WARN, L_SCAN_SCANNER, "Could not decode vorbis header "
	...
	if (ogg_page_granulepos(page) != 0 || ogg_stream_packetpeek(&stream->os, NULL) == 1)
	  DPRINTF(E_WARN, L_SCAN_SCANNER, "No header in vorbis stream %d\n", stream->num);
	DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Vorbis headers parsed for stream %d, "
		"information follows...\n", stream->num);
	DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Channels: %d\n", inf->vi.channels);
	DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Rate: %ld\n\n", inf->vi.rate);

	psong->samplerate = inf->vi.rate;
	psong->channels = inf->vi.channels;

	if (inf->vi.bitrate_nominal > 0) {
	  DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Nominal bitrate: %f kb/s\n",
	  ...
	  DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Nominal bitrate not set\n");
	  ...
	  if (inf->vi.bitrate_upper > 0) {
	    DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Upper bitrate: %f kb/s\n",
		    (double)inf->vi.bitrate_upper / 1000.0);
	    upper_rate = inf->vi.bitrate_upper;
	  }
	  else {
	    DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Upper bitrate not set\n");
	  }

	  if (inf->vi.bitrate_lower > 0) {
	    DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Lower bitrate: %f kb/s\n",
		    (double)inf->vi.bitrate_lower / 1000.0);
	    lower_rate = inf->vi.bitrate_lower;;
	  }
	  else {
	    DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Lower bitrate not set\n");
	  }
	...
	if (inf->vc.comments > 0)
	  DPRINTF(E_DEBUG, L_SCAN_SCANNER,
  ...
  if (!header) {
	...
	DPRINTF(E_WARN, L_SCAN_SCANNER, "granulepos in stream %d decreases from %lld to %lld",
		stream->num, inf->lastgranulepos, gp);
      inf->lastgranulepos = gp;
    }
    else {
      DPRINTF(E_WARN, L_SCAN_SCANNER, "Malformed vorbis strem.\n");
    }
...
_ogg_find_stream_processor(ogg_stream_set *set, ogg_page *page)
{
...

  {
    ...
    if (res <= 0) {
      DPRINTF(E_WARN, L_SCAN_SCANNER, "Invalid header page, no packet found\n");
    ...
    res = ogg_stream_packetout(&stream->os, &packet);
    if (res > 0) {
      DPRINTF(E_WARN, L_SCAN_SCANNER, "Invalid header page in stream %d, "
	      ...
	      ogg_int64_t *written)
{
  ...
  while ((ret = ogg_sync_pageout(sync, page)) <= 0) {
    if (ret < 0)
      DPRINTF(E_WARN, L_SCAN_SCANNER, "Hole in data found at approximate offset %lld bytes. Corrupted ogg.\n", *written);
...
_get_oggfileinfo(char *filename, struct song_metadata *psong)
{
  ...
  DPRINTF(E_INFO, L_SCAN_SCANNER, "Processing file \"%s\"...\n\n", filename);
  ...
  while (_ogg_get_next_page(file, &sync, &page, &written)) {
    ...
    if (p->isnew) {
      DPRINTF(E_DEBUG, L_SCAN_SCANNER, "New logical stream (#%d, serial: %08x): type %s\n",
	...
	DPRINTF(E_WARN, L_SCAN_SCANNER,
      ...
      DPRINTF(E_WARN, L_SCAN_SCANNER, "stream start flag found in mid-stream "
	      "on stream %d\n", p->num);
	...
	DPRINTF(E_DEBUG, L_SCAN_SCANNER, "Logical stream %d ended\n", p->num);
	p->isillegal = 1;
  ...
  if (!gotpage) {
    DPRINTF(E_ERROR, L_SCAN_SCANNER, "No ogg data found in file \"%s\".\n", filename);
    return -1;
  }
