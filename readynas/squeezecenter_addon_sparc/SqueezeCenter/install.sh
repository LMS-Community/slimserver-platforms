...
dpkg -i --force-all squeezecenter-readynas*.deb &>/dev/null || bye "ERROR: $friendly_name installation failed"
>/etc/debian_version && chattr +i /etc/debian_version
cp squeezecenter /etc/init.d/squeezecenter

# Symlink the new log file to the old location, so the log .zip file picks it up
rm -f /var/log/slimserver.log
ln -sf /var/log/squeezecenter/server.log /var/log/slimserver.log 
